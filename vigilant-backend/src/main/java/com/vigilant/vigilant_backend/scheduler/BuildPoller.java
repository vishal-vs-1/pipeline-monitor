...
public class BuildPoller {
    ...
    /**
     * Handles an existing workflow run. If the status (e.g. in_progress -> completed)
     * or conclusion (e.g. success -> failure) has changed, updates the DB and fires a Kafka event.
     */
    private void handleExistingBuildState(TrackedRepo repo, WorkflowRun latestRun, Integer durationSeconds, BuildState existingState) {
        boolean statusChanged = !String.valueOf(existingState.getStatus()).equals(String.valueOf(latestRun.status()));
        boolean conclusionChanged = !String.valueOf(existingState.getConclusion()).equals(String.valueOf(latestRun.conclusion()));
    ...
    /**
     * Constructs a BuildEvent record and publishes it to the 'build-events' Kafka topic.
